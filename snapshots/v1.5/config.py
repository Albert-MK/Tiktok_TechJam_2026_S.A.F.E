...
正式评分默认使用 PRESETS["final"]，也就是当前 v1.5。
"""
...
PRESETS = {
    ...
    #   6) v1.4：首轮若还没有强约束，先不交 Top-10；
    #   7) v1.5：约束少于 2 条时最多再空一轮；title BM25 权重 6→4。
    # ---------------------------------------------------------------------
    "final": {
        ...
        "delay_until_n_constraints": 2,     # v1.5：约束未满 2 条时再等
        "delay_max_empty": 2,               # v1.5：最多空 2 轮
        "bm25_field_weights": [4.0, 4.0, 2.5, 2.5, 1.5, 1.0],  # v1.5：降低 title
    },
}
...
def active_config() -> dict:
    ...
    cfg.setdefault("delay_max_empty", 0)
    cfg.setdefault("delay_until_n_constraints", 0)
    cfg.setdefault("delay_min_margin", 2.0)
    cfg.setdefault("bm25_field_weights", [6.0, 4.0, 2.5, 2.5, 1.5, 1.0])
    return cfg
